...
class TikTokScraper extends EventEmitter {
                    ...
                    if (store[item.authorName]) {
                        ...
                        this.request({
                            ...
                            .then(result => {
                                if (result === '{"statusCode":200,"contentType":"text/html","content":""}') {
                                ...
                                } else if (result.indexOf('<script id="__NEXT_DATA__" type="application/json" crossorigin="anonymous">') > -1) {
                                    ...
                                    store[item.authorName] = newResult.props.pageProps.userData;
                                    const { following, fans, heart, video, digg, verified } = store[item.authorName];
                                    item.authorFollowing = following;
                                    item.authorFans = fans;
                                    item.authorHeart = heart;
    ...
    getHashTagId() {
        return new Promise(async (resolve, reject) => {
            try {
                resolve(
                    this.getIds({
                        uri: `${this.mainHost}tag/${this.input}`,
                        type: 'hashtag',
                    }),
                );
            } catch (error) {
    ...
    getIds({ uri, type }) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                let body = await this.request(query);
