...
class TikTokScraper extends EventEmitter {
    constructor({ download, filepath, filetype, proxy, asyncDownload, cli, event, timeout, progress, input, number, type, by_user_id = false }) {
        super();
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 0; i < posts.length; i++) {
                ...
                let item = {
                    ...
                    authorName: posts[i].musicInfos.authorName,
    ...
    _scrapeData(qs, maxCursor) {
        return new Promise(async (resolve, reject) => {
            ...
            if (this._scrapeType === 'music') {
                _signature = generateSignature(
                    `https://www.tiktok.com/share/item/list?secUid=${
                    qs.secUid
                    }&id=${qs.id}&type=${qs.type}&count=${qs.count}&minCursor=${
                    qs.minCursor
                    }&maxCursor=${maxCursor || 0}&shareUid=&lang=en`
                );
            } else {
                let shareUid = qs.type === 5 ? '&shareUid=' : '';
                _signature = generateSignature(
                    `https://www.tiktok.com/share/item/list?secUid=${
                    qs.secUid
                    }&id=${qs.id}&type=${qs.type}&count=${qs.count}&minCursor=${
                    qs.minCursor
                    }&maxCursor=${maxCursor || 0}${shareUid}`
                );
            }
