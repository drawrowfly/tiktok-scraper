...
class TikTokScraper extends EventEmitter{
    ...
    _addBar(len) {
        this._progressBar.push(this._mbars.newBar('Downloading :id [:bar] :percent', {
            complete: '=', 
            incomplete: ' ', 
            width: 30, 
            total: len
                ...
                try{
                    switch(this._scrapeType){
                        ...
                        case 'user':
                            ...
                            break;
                        case 'trend':
                            var result = await this._scrapeData({ id: '', secUid: '', shareUid: '', type: 5, count: 48, minCursor: 0 }, maxCursor)
                            break;
                    }
    ...
    _getIds({ uri, type }){
        return new Promise( async (resolve, reject) => {
            ...
            try{
                let body = await this._request(query);
                body = JSON.parse(body.split('window.__INIT_PROPS__ = ')[1].split('</script>')[0]);
                if (type === 'user'){
                    if (body['/@:uniqueId'].statusCode !== 0){
                        return reject({ message: `Can't find anything` })
                    }
                    resolve({ id: body['/@:uniqueId'].userData.userId, secUid: body['/@:uniqueId'].userData.secUid, type: 1, count: 48, minCursor: 0 });
                }
                if (type === 'hashtag'){
                    if (body['/tag/:name'].statusCode !== 0){
                        return reject({ message: `Can't find anything` })
                    }
                    resolve({ id: body['/tag/:name'].challengeData.challengeId, secUid: '', type: 3, count: 48, minCursor: -1 });
                }
            } catch(error){
                reject(error)
            }
    ...
    _getHashTagId(){
        return new Promise( async (resolve, reject) => {
            try{
                resolve(this._getIds({ uri: `${this._mainHost}tag/${this._input}`, type: 'hashtag' }))
            } catch(error){
                reject(error)
            }
        })
    ...
    _getUserId(){
        return new Promise( async (resolve, reject) => {
            try{
                resolve(this._getIds({ uri:  `${this._mainHost}@${this._input}`, type: 'user' }))
            } catch(error){
                reject(error)
            }
        })
