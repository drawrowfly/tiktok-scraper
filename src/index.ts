import { TikTokScraper } from './core/TikTok';
...
const promiseScraper = async (input: string, type: ScrapeType, options?: Options): Promise<Result> => {
    ...
    const contructor: TikTokConstructor = Object.assign({ ...INIT_OPTIONS, ...{ type, input } }, options);
...
const eventScraper = (input: string, type: ScrapeType, options?: Options): TikTokScraper => {
    ...
    const contructor: TikTokConstructor = Object.assign({ ...INIT_OPTIONS, ...{ type, input, event: true } }, options);
...
export const getHashtagInfo = async (input: string, options?: Options): Promise<Challenge> => {
    if (options && typeof options !== 'object') {
        throw 'Object is expected';
    }
    const contructor: TikTokConstructor = Object.assign({ ...INIT_OPTIONS, ...{ type: 'signle_hashtag' as ScrapeType, input } }, options);
    const scraper = new TikTokScraper(contructor);
    try {
        return await scraper.getHashtagInfo();
    } catch (error) {
        throw error;
    }
};
...
export const getUserProfileInfo = async (input: string, options?: Options): Promise<UserData> => {
    ...
    const contructor: TikTokConstructor = Object.assign({ ...INIT_OPTIONS, ...{ type: 'sinsgle_user' as ScrapeType, input } }, options);
    const scraper = new TikTokScraper(contructor);
    try {
        return await scraper.getUserProfileInfo();
    } catch (error) {
        throw error;
    }
};
export const signUrl = async (input: string, options?: Options): Promise<string> => {
    ...
    const contructor: TikTokConstructor = Object.assign({ ...INIT_OPTIONS, ...{ type: 'signature' as ScrapeType, input } }, options);
    const scraper = new TikTokScraper(contructor);
    try {
        return await scraper.signUrl();
    } catch (error) {
        throw error;
    }
};
